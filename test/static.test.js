...
test('el catálogo y lo que hace falta para buscar en él sí se sirven', () => {
  ...
  assert.equal(publicPath('/src/game/worlds.js'), 'src/game/worlds.js');
...
// Criterio 8 de la v4: añadir un mundo no puede exigir acordarse de publicar su
// fichero, así que la lista blanca sale del registro.
test('se sirve el catálogo de todos los mundos, no solo el primero', () => {
  for (const world of WORLDS) {
    const path = catalogPath(world.id);
    assert.equal(publicPath(`/${path}`), path, `${world.name} no se está sirviendo`);
  }
});
