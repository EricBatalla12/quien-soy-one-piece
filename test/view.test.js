...
function wire(room, playerId) {
  return JSON.stringify(projectView(room, playerId));
}

// ---------------------------------------------------------------------------
// Criterio 5: el secreto es secreto de verdad
// ---------------------------------------------------------------------------

test('tu personaje no viaja en ningún momento de la partida', () => {
  const room = playingRoom();

  assert.ok(!wire(room, 1).includes('Nico Robin'), 'el jugador 1 no puede ver quién es');
  assert.ok(!wire(room, 2).includes('Zoro'), 'el jugador 2 no puede ver quién es');
});

test('tu personaje tampoco viaja mientras el rival lo escribe', () => {
  const half = withGame(playingRoom(), setSecret(createGame(), 2, 'Nico Robin'), NOW);

  assert.ok(!wire(half, 1).includes('Nico Robin'));
});

test('el personaje que escribiste tú sí lo ves', () => {
  const view = projectView(playingRoom(), 1);

  assert.equal(view.chosenForRival, 'Zoro');
});
...
test('sabes si el rival ya ha elegido, pero no qué ha elegido', () => {
  ...
  assert.equal(projectView(room, 1).rivalHasChosen, true);
  assert.equal(projectView(empty, 1).rivalHasChosen, false);
  assert.equal(projectView(room, 1).yourCharacter, null);
});

test('al terminar se revelan los dos personajes', () => {
  const room = withGame(playingRoom(), guess(playingRoom().game, 1, 'Nico Robin'), NOW);
  const view = projectView(room, 1);
...
test('un intento fallido no revela el personaje, solo lo que se intentó', () => {
  const room = withGame(playingRoom(), guess(playingRoom().game, 1, 'Sanji'), NOW);
  const view = projectView(room, 1);

  assert.equal(view.phase, 'playing');
  assert.equal(view.yourCharacter, null);
  assert.equal(view.history.at(-1).text, 'Sanji');
...
test('sin rival la sala está esperando, no en preparación', () => {
  ...
  const view = projectView(alone, 1);
...
test('la vista trae los nombres y de quién es el turno', () => {
  const view = projectView(playingRoom(), 2);
...
test('la vista dice si el rival se ha caído', () => {
  ...
  assert.equal(projectView(room, 2).rival.connected, false);
  assert.equal(projectView(room, 1).rival.connected, true);
});
...
test('la pregunta pendiente y el historial se ven igual desde los dos lados', () => {
  ...
  assert.deepEqual(projectView(pending, 1).pendingQuestion, {
    from: 1,
    text: '¿Eres espadachín?',
  });
  assert.deepEqual(projectView(pending, 2).pendingQuestion, projectView(pending, 1).pendingQuestion);
  assert.deepEqual(projectView(answered, 1).history, projectView(answered, 2).history);
  assert.equal(projectView(answered, 1).pendingQuestion, null);
});

test('el marcador se ve entero', () => {
  const room = { ...playingRoom(), score: { 1: 2, 2: 1 } };

  assert.deepEqual(projectView(room, 1).score, { 1: 2, 2: 1 });
});

test('no hay vista de una plaza vacía', () => {
  const alone = createRoom({ code: 'NAKAM', name: 'Eric', token: 't1', now: NOW });

  assert.throws(() => projectView(alone, 2), /libre/);
});
