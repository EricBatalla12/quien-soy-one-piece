...
/**
 * Sala recién creada: quien la abre ocupa la plaza 1, con el mundo que ha elegido, y
 * espera rival.
 *
 * El mundo es de la sala y no de cada jugador (sección 4 de la espec v4): quien entra
 ...
 * mundo que no existe no tendría catálogo con el que validar ni un solo personaje.
 */
export function createRoom({ code, name, token, world, now }) {
  if (!isWorldId(world)) throw new Error('Ese mundo no existe');
