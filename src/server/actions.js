 ...
 * Las reglas de la partida siguen siendo las de la v1, intactas: quien las hace
 * cumplir ahora es el servidor, así que un jugador no puede saltárselas mandando la
 * acción a mano por el WebSocket (criterio 4).
 */

import { answerQuestion, askQuestion, guess, setSecret } from '../game/state.js';
import { isFull, rematch, withGame } from './rooms.js';

export function applyAction(room, playerId, message, now) {
  if (!isFull(room)) throw new Error('Espera a que entre tu rival');

  switch (message.type) {
    case 'secret':
      return withGame(room, setSecret(room.game, playerId, message.text), now);
    case 'ask':
    ...
    case 'guess':
      return withGame(room, guess(room.game, playerId, message.text), now);
    case 'rematch':
